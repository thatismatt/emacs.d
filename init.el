...
(use-package org
  :defer t
  :init
  (defface org-doing-face '((t (:inherit org-todo))) "org mode face for DOING items" :group 'org)
  (defface org-postponed-face '((t (:inherit org-done))) "org mode face for POSTPONED items" :group 'org)
  (defface org-query-face '((t (:inherit org-todo))) "org mode face for ??? (query) items" :group 'org)
...
(defun matt-scroll-mode ()
  ...
  (interactive)
  (let* ((map       (make-sparse-keymap))
         (exit-fun  (set-transient-map map t (lambda () (hl-line-mode -1)) "Scroll! %k"))
         (exit-mode (lambda () (interactive) (funcall exit-fun)))
    ...
    (define-key map "q" exit-mode)
    (define-key map [return] exit-mode)
...
e.g. 2020012016131337, 2020-01-20_16-13-13, 1621854380123 or 1621854380."
  ...
  (insert (format-time-string
           (cond
            ((eq arg 256) "%s")                ;; 1621854575
            ((eq arg 64)  "%s%3N")             ;; 1621854572121
            ((eq arg 16)  "%F %T")             ;; 2021-05-24 12:03:45
